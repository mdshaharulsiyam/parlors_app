...
export const useAddressApiCall = ({
  ...
  const {data: divisions, isLoading: isLoadingDiv} = useGetDivisionsQuery({
    page: 1,
  ...
  const {data: upazilas, isLoading: upazillaLoading} = useGetUpazilasQuery({
    district_id,
    search: upazillaSearch,
  });
  const {data: unions, isLoading: unionLoading} = useGetUnionsQuery({
    upazilla_id,
    search: unionSearch,
  });
